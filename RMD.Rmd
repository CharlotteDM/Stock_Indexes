...
The number of differentiations was determined using the ndiffs function and amounted to 0. 


The ARMA function showed that the optimal model for the discussed time model will be ARMA (1, 0, 0).
...
The predictions made by all methods were compared. MAPE turned out to be the smallest in the __ARIMA model__.

The ols_step_all_possible function allows the assessment of how different models account for VIX variability. For example if only the CDS5Y variable was taken as a predictor - it explains 57% of the VIX variability. Ols_step_both_aic function allows to observe regression using stepwise method. 
The histogram confirms a normal distribution. The K-S test (performed with the use of the ols test normality function) allows to state that the residuals are normally distributed, which is also confirmed by the quantil-quantil plot. Outliers were determined using the Cook’s distance.
...
* https://www.statology.org/dickey-fuller-test-in-r/ (01.07.2022)